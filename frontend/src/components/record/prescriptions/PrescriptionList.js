...
export default function PrescriptionList({ patientId, isDoctor, createSignal = 0 }) {
  ...
  const buildUrl = () => {
    ...
    const base = `${API_BASE}/api/prescriptions/patient/${encodeURIComponent(patientId)}`;
    const s = qs.toString();
    ...
    <Box>
      ...
      {nothingFound && (
        <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
          {filters.q.trim()
            ? <>No prescriptions match "<strong>{filters.q}</strong>"</>
      ...
      <Stack
        spacing={2}
        sx={{ opacity: loading ? 0.7 : 1, transition: "opacity .2s" }}
      >
        {items.map((it) => (
