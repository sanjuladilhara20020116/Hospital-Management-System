...
export default function AdmissionList({ patientId, isDoctor, createSignal = 0 }) {
  ...
  const buildUrl = () => {
    ...
    const base = `${API_BASE}/api/admission-notes/patient/${encodeURIComponent(patientId)}`;
    const s = qs.toString();
    ...
    <Box>
      ...
      {nothingFound && (
        <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
          {filters.q.trim()
            ? <>No admission notes match "<strong>{filters.q}</strong>"</>
      ...
      {/* View */}
      {openView && <AdmissionViewDialog item={openView} onClose={() => setOpenView(null)} />}
