...
export default function PatientDashboard({ userId }) {
  ...
  // Edit / Delete appointment dialogs
  const [editDialogOpen, setEditDialogOpen] = useState(false);
  const [editAppointment, setEditAppointment] = useState(null);
  const [editSlotDuration, setEditSlotDuration] = useState(0); // minutes
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const res = await axios.get(`http://localhost:5000/api/appointments/patients`, {
  ...
  useEffect(() => {
    fetchAppointments();
  }, [userId]); // eslint disabled earlier in your codebase
  ...
  const handleEditAppointment = async (appointment) => {
    ...
    try {
      const res = await axios.get('http://localhost:5000/api/appointments/doctors/slots', {
  ...
  const handleSaveEditAppointment = async () => {
    ...
    try {
      ...
      const msg = e?.response?.data?.message || 'Failed to reschedule appointment';
      showAlert('error', msg);
    }
  };
  ...
  return (
    <>
      <Container
        ...
        sx={{ py: 4, px: { xs: '12px', sm: '2.5vw', lg: '24px' } }}
      >
        <Card elevation={3} sx={{ borderRadius: 3 }}>
          <CardContent sx={{ p: 0 }}>
              ...
              {editMode ? (
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={6}>
                    <Paper elevation={0} sx={{ p: 2, borderRadius: 2, backgroundColor: '#f5f7fb' }}>
            ...
            {!editMode && (
              <Box sx={{ px: 3, pb: 1 }}>
                ...
                <Box className="pd-actions-tiles">
                  <button className="pd-tile pd-tile--blue" onClick={() => navigate('/my-reports')}>
                    <span className="pd-tile-icon"><ScienceOutlined /></span>
                    <span className="pd-tile-text">Lab Reports</span>
                  </button>

                  <button className="pd-tile pd-tile--yellow" onClick={() => navigate('/patient/medical-records')}>
                    <span className="pd-tile-icon"><DescriptionIcon /></span>
                    <span className="pd-tile-text">Medical Reports</span>
                  </button>

                  <button className="pd-tile pd-tile--green" onClick={() => navigate('/packages')}>
                    <span className="pd-tile-icon"><MedicalIcon /></span>
                    <span className="pd-tile-text">Health Pakages</span>
                  </button>

                  <button className="pd-tile pd-tile--red" onClick={() => navigate('/appointments')}>
                    <span className="pd-tile-icon"><AppointmentIcon /></span>
                    <span className="pd-tile-text">Book Appointment</span>
                  </button>

                  <button className="pd-tile pd-tile--sky" onClick={() => navigate('/vaccinations/mine')}>
                    <span className="pd-tile-icon"><VaccinesIcon /></span>
                    <span className="pd-tile-text">Vaccination</span>
                  </button>
                </Box>
              </Box>
            )}
            ...
            {!editMode && (
              <Box sx={{ px: 3, pb: 3 }}>
                ...
                <Box sx={{ mb: 1.5, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 1.5 }}>
                  <TextField
                      ...
                      <Grid item xs={12} key={a._id || a.referenceNo}>
                        <Paper
                          ...
                          sx={{
                            ...
                            flexWrap: 'wrap', gap: 2,
                            width: '100%',
                            mx: 'auto', cursor: 'pointer', borderColor: '#e6eaf2'
                          }}
          ...
          {/* FOOTER: single source of truth for edit actions */}
          <CardActions sx={{ p: 3, pt: 0, justifyContent: 'flex-end', gap: 1.5 }}>
            {editMode ? (
              <>
                <Button
                  variant="contained"
      ...
      <Dialog open={detailsDialogOpen} onClose={() => setDetailsDialogOpen(false)} maxWidth="sm" fullWidth>
        {selectedAppointment && (
          <Box sx={{ background: 'linear-gradient(120deg, #fdfefe 0%, #f5f7fa 100%)', borderRadius: 2 }}>
            ...
            <DialogContent dividers sx={{ p: 3 }}>
              <Stack alignItems="center" spacing={1} mb={2}>
                ...
                <Chip
                  ...
                  color={
                    selectedAppointment.status === 'Cancelled' ? 'default' :
                    selectedAppointment.status === 'Confirmed' ? 'success' : 'warning'
                  }
              ...
              <Box sx={{ mb: 2, p: 2, borderRadius: 2, background: '#e3f0ff' }}>
                <Typography variant="h6" fontWeight={700} color="primary" sx={{ mb: 1 }}>
                  <LocalHospitalIcon sx={{ mr: 1, mb: -0.5 }} />Doctor Details
                </Typography>
              ...
              <Box sx={{ mb: 2, p: 2, borderRadius: 2, background: '#f5f7fa' }}>
                <Typography variant="h6" fontWeight={700} color="primary" sx={{ mb: 1 }}>
                  <PersonIcon sx={{ mr: 1, mb: -0.5 }} />Patient Details
                </Typography>
              ...
              <Box sx={{ mb: 2, p: 2, borderRadius: 2, background: '#e3f0ff' }}>
                <Typography variant="h6" fontWeight={700} color="primary" sx={{ mb: 1 }}>
                  <CalendarMonthIcon sx={{ mr: 1, mb: -0.5 }} />Scheduled Date &amp; Time
                </Typography>
              ...
              <Box sx={{ mb: 2, p: 2, borderRadius: 2, background: '#f5f7fa' }}>
                <Typography variant="h6" fontWeight={700} color="primary" sx={{ mb: 1 }}>
                  <CreditCardIcon sx={{ mr: 1, mb: -0.5 }} />Payment Details
                </Typography>
          ...
          {appointmentToDelete && (
            <Box sx={{ mt: 2 }}>
              ...
              <Typography variant="body2">
                Date: {appointmentToDelete.date} {appointmentToDelete.startTime} - {appointmentToDelete.endTime}
              </Typography>
              <Typography variant="body2">
                Doctor: {appointmentToDelete.doctorName || appointmentToDelete.doctorId}
              </Typography>
            </Box>
          )}
        </DialogContent>
